...
- ポイント一覧(`#point-list`)の項目をクリックすると、編集モーダルは開かず`map.setView([point.lat, point.lng], 17)`で地図がそのポイントへ移動し、`markers`(ポイントID→マーカーの`Map`)から対応マーカーを取得してポップアップを開く(ポイント数が多い時にどれがどこか確認しやすくするため)。編集は引き続き地図上のマーカークリックから行う(`renderMarkers`内の`marker.on("click", ...)`は変更していない)
